...
- **Government** is its own vertical served by USAJobs, not Workday (agencies don't post on the curated tenants). `public_sector_government` → `{government}` → `_discover_government_jobs` → `usajobs_client`. The USAJobs client is gated on `NEXUSREACH_USAJOBS_API_KEY` + `NEXUSREACH_USAJOBS_USER_AGENT` and **fails soft** to `[]` when unset, so government discovery still works via the broad aggregators and simply gains the official federal board when the key is present.
