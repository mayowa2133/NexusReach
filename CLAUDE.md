...
- People discovery is **pre-warmed and cache-served** so "Find People" feels instant:
  - On job discovery, `jobs/storage._maybe_prewarm_people` queues a discovery-only `auto_prospect.prewarm_company_people` Celery task for the top 10 distinct companies (ranked by best `match_score`) in the stored batch. It calls `search_people_at_company(..., persist=False)` — warming only the **global** known-people cache, never persisting `Person` CRM rows — and skips companies already holding ≥8 cached contacts. On by default (`people_prewarm_enabled` setting, opt-out in the Auto-Prospect panel); never finds emails / drafts / sends.
...
34. **People pre-warm and snapshot serving never cross the global-cache / CRM boundary.** `search_people_at_company(persist=False)` warms only the GLOBAL `known_people` cache — an on-by-default background warm must never silently fill a user's saved `Person` rows; CRM rows are persisted only on a user-initiated Find People. The response/snapshot serialization shared by the `/people/search` handler and the `auto_prospect.refresh_job_research_snapshot` task lives in `app/services/people/serialize.py` (single source of truth — import/patch it there, not the router). Snapshot freshness windows (`SNAPSHOT_FRESH_TTL` 24h, `SNAPSHOT_MAX_SERVE_AGE` 14d) and the serve/refresh/miss decision live in `job_research_snapshot_service.snapshot_serve_decision`. The pre-warm and refresh tasks live in `app/tasks/auto_prospect.py` and are event-triggered via `.delay` (no beat schedule).
