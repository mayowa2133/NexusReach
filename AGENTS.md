...
- **`security/remediation-2026-09`** — **merged and deleted.** Listed only because the September 2026 audit remediation is worth finding by name and there is no branch left to check out: it is on `main` as merge commit `99c86cf4` (PR #40), and the remediation itself is commit `3b59e4ac` plus migrations `068`–`074` — renderer isolation, paid-work reservations, durable deletion receipts, client-capture provenance, the send-attempt ledger, referral credential/campaign tables, JWT claim validation, and least-privilege grants on internal tables. Everything marked "September 2026" in this file is on `main`. Follow-up commits on that branch also fixed three places where the hardening outran its own test scaffolding (an unpinned issuer in a JWT unit test, a missing `iss` claim in the E2E token, and first-time bootstrap having no Supabase admin API to consult in the harness), and named the security job `Security` so it matches the status check `main`'s branch protection requires — before that, the required context did not exist and no pull request into `main` could merge.
- **`claude/agitated-snyder-8c4bb6`** — **stale, kept deliberately.** Five unmerged commits from May 2026 (billing/freemium, landing page, onboarding wizard, ToS/privacy pages, circuit breaker + Sentry). Parts of it shipped by other routes — `main` has the landing and policy pages today — so treat it as possibly-duplicated work, not a backlog. It predates the jobs-package refactor, so rebasing it is real work. Not a work item unless someone decides it is.
...
- **Waitlist fallback accounting (September 2026; supersedes older fallback/analytics wording below).** A Google Sheets `no-cors` fallback request is opaque, so the landing page reports it as an unconfirmed backup attempt and never emits `waitlist_joined`; only a successful backend `202` counts as a confirmed join in product analytics.
- **Landing and waitlist conversion update (September 2026; supersedes the older form details in the next bullet).** The public landing page now leads with the outcome “Find the people behind your next opportunity,” uses a compact interactive Engineering/Marketing example, separates role relevance, company evidence, and email confidence, labels all sample people and companies as illustrative, and describes optional delayed sending accurately. The public modal collects **email only**. `WaitlistSignupCreate.name` and `waitlist_signups.name` are nullable via migration `075_waitlist_optional_name`; name and target occupation move to the verified referral dashboard. `PATCH /api/referrals/profile` accepts those optional fields only with the owner bearer token and only after email confirmation. The anonymous `POST /api/waitlist` remains generic and insert-only for an existing address. External landing fonts and the inline theme initializer were removed so the shipped Content Security Policy no longer blocks them; Geist is bundled and `/theme-init.js` is same-origin. The footer renders a contact link only when `VITE_CONTACT_EMAIL` is configured.
