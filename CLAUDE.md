...
3. **Email waterfall stops early** — Apollo Enrichment → Hunter → Proxycurl → Hunter Domain. First verified email wins. Falls back to LinkedIn if nothing found.
...
9. **Apollo has TWO search endpoints** — `/api/v1/mixed_people/api_search` (free, no emails, master key in header) vs `/v1/mixed_people/search` (credits, returns emails, key in body). We use the free one for discovery. Enrichment uses `/v1/people/match` (1 credit per email, standard key in body).
